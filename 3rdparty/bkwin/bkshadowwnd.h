	...
	void OnMove(HWND hParent, WPARAM wParam, LPARAM lParam)
	{
		if (!m_bMoving && m_Status & SS_VISABLE)
		{
			...
			if (WndRect.bottom - WndRect.top == 0 || WndRect.right - WndRect.left == 0)
			{
				...
				WndRect.left - SHADOWSIZE, WndRect.top - SHADOWSIZE,
...
protected:
	...
	void Update(HWND hParent)
	{
		if ((HWND)INVALID_HANDLE_VALUE != m_hWnd)
		{
			RECT WndRect;
			GetWindowRect(hParent, &WndRect);
			int nShadWndWid = WndRect.right - WndRect.left + SHADOWSIZE * 2;
			int nShadWndHei = WndRect.bottom - WndRect.top + SHADOWSIZE * 2;
			...
			POINT ptDst = {WndRect.left - SHADOWSIZE, WndRect.top - SHADOWSIZE};
			POINT ptSrc = {0, 0};
...

			{
				...
				if (pSkin)
				{
					...
					int n = (szSkin.cx - 2)/2; 
					pSkin->DrawImage(hMemDC, CRect(0, 0, n, n),CRect(0, 0, n, n));
					pSkin->DrawImage(hMemDC, CRect(rcShadow.right - n, 0, rcShadow.right, n), CRect(szSkin.cx - n, 0, szSkin.cx, 9));
					pSkin->DrawImage(hMemDC, CRect(0, rcShadow.bottom - n, n, rcShadow.bottom), CRect(0, szSkin.cy - n, n, szSkin.cy));
					...
					pSkin->DrawImage(hMemDC, CRect(n, 0, rcShadow.right - n, n), CRect(n, 0 , n + 1, n));
					pSkin->DrawImage(hMemDC, CRect(n, rcShadow.bottom - n, rcShadow.right - n, rcShadow.bottom), CRect(n, szSkin.cy - n, n + 1, szSkin.cy));
					pSkin->DrawImage(hMemDC, CRect(0, n, n, rcShadow.bottom - n), CRect(0, n , n, n + 1));
					pSkin->DrawImage(hMemDC, CRect(rcShadow.right - n, n, rcShadow.right, rcShadow.bottom - n), CRect(szSkin.cx - n, n, szSkin.cx, n + 1));
				}
			}
