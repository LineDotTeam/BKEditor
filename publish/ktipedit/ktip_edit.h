...
#define WM_RENTER_DOWN              (WM_USER + 7)
...
public:
    ...
    {
        if (m_hNotify && wParam == VK_RETURN)
        {
            ::SendMessage(m_hNotify, WM_RENTER_DOWN, wParam, 0);
        }
...
public:
	...
	{
		m_bTiping = TRUE;
		m_cfTip = RGB(0xff, 0xff, 0xff);
		m_hNotify = NULL;
		m_nxMargin = 0;
		m_nyMargin = 0;
		m_nullBrush = CreateSolidBrush(RGB(0x00, 0x71, 0xB5));
		m_nStyle = 0;
	}
	...
	BOOL OnEraseBkgnd(CDCHandle dc)
	{
		...
		writeBrush.CreateSolidBrush(RGB(0x00, 0x71, 0xB5));
    ...
    LRESULT OnRenterDown(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL &bHandled)
    {
        if (m_hNotify)
        {
            ::SendMessage(m_hNotify, WM_RENTER_DOWN, wParam, 0);
	...
	BEGIN_MSG_MAP_EX(KTipEdit)
        ...
        MESSAGE_HANDLER(WM_RENTER_DOWN, OnRenterDown)
		REFLECT_NOTIFICATIONS_EX()
	END_MSG_MAP()
protected:
	void DoActive()
