...
KEditDialogImpl::KEditDialogImpl(HINSTANCE hInstance, KEditDialogView* pEditView)
...
void KEditDialogImpl::OnPalette()
{
    ...
    if (m_uSelItemID != 0)
    {
        ...
        if (pCompTmp != NULL)
        {
            ...
            m_pEditView->SetItemCheck(m_uSelItemID, FALSE);
            m_uSelItemID = 0;
            m_pNowComp   = pCompTmp;
...
void KEditDialogImpl::OnUninitDialog()
{
    ...
    m_vecComp.clear();
    m_pEditView->EndDialog(0);
}

void KEditDialogImpl::OnMinSize()
{
    ::ShowWindow(m_pEditView->m_hWnd, SW_SHOWMINIMIZED);
}
...
void KEditDialogImpl::_RedrawComponent()
{
    ...
    m_pEditView->SetPanelXml(MAIN_OPERATE, strNowXml.GetString());
}
...
void KEditDialogImpl::_RedrawAttrute()
{
    ...
    if (m_pNowComp != NULL)
    {
        ...
        m_pEditView->SetPanelXml(ATTR_OPERATE, strNowXml.GetString());
    ...
    m_pEditView->SetItemVisible(SUBMIT_BUTTION, bBtnVisible);
    m_pEditView->SetItemVisible(DELETE_BUTTION, bBtnVisible);
}
...
void KEditDialogImpl::_InitEdit()
{
    ...
    {
        m_kEdit[i].SetNotify(m_pEditView->m_hWnd);
        m_kEdit[i].SetStyle(ES_MULTILINE | ES_WANTRETURN);
        m_kEdit[i].Create(m_pEditView->GetViewHWND(), NULL, NULL, WS_CHILD, 0, enAttruteEdit_Begin + i);
